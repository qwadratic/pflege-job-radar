...
1. Read Backlog.md workflow guidance first:
   ...
   (Without the Backlog.md MCP server these are `backlog instructions overview`, `backlog instructions task-creation`,
   `backlog instructions task-execution`, `backlog instructions task-finalization` — same text, CLI door.)
...
```bash
mkdir -p ../pflege-board-copies
if [ ! -d "../pflege-board-copies/pflege-board-<taskId>/.git" ]; then
  git clone "$(pwd)" "../pflege-board-copies/pflege-board-<taskId>"
fi
cd "../pflege-board-copies/pflege-board-<taskId>"
...
2. **Using local-path git remotes in dedicated clones**
   ...
   - TPM guardrail: standardize clone remote before PR steps:
     - `git remote set-url origin https://github.com/qwadratic/pflege-job-radar.git`
