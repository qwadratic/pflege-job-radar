...
2026-09-22 remediation round (reviewer findings #1 and #4): fixed the regression and redid the dry run.

Finding #1 (regression, priority): pflege_jobs/inbox_db.py loaded_refs() returned inbox.source_url while posting_observations.source_ref is canonical (TASK-83) for kind=observation rows -- app/crawl.py's _posting_ids_for_refs filters on source_ref, so every softgarden/dvinci/umantis/helix/b-ite career_crawl row's post-crawl posting_id lookup silently found nothing (0 postings touched/new logged, _verify_ids never ran for them). Fixed by reading source_ref out of the row's own stored payload (falls back to source_url for kind=jobposting rows, which never carry that field). Both sides now key on (source_id, source_ref) -- the table's real unique identity -- chosen over changing the consumer to source_url, which is non-unique and would keep missing URL-shape variants. Test: tests/test_inbox_sqlite_queue.py::test_loaded_refs_keys_an_observation_row_on_its_own_source_ref_not_source_url, red against the pre-fix code, confirmed via a /tmp-backed revert-and-restore (not git).

Finding #4: rewrote backups/task83_dedup_dryrun_script_20260922T024753Z.py to group posting_observations directly by (source_id, canonical_job_url(source_ref)) instead of (clinic_id, canonical_job_url(external_url)) off postings/v_postings -- clinic_id does not exist on posting_observations and cannot see a duplicate spanning two different clinics by construction. Re-ran live (read-only) just now: 2576 open postings today, 23 duplicate groups, -23 (2576->2553); found and named the real cross-clinic collision live (helix key covering posting_id 7542/clinic 66104 and 10267/clinic 67804), and traced that edge/pflege-ingest/index.ts's own merges op would drop clinic 67804's row entirely (coalesce(dst.clinic_id, src.clinic_id), dst already set). Named-clinic counts re-verified live: Bamberg/Fuerth/Neumarkt -5 each, Bayreuth 0, Kempten 0 (unchanged from the original finding). New report+pairs written alongside the original (not overwritten): backups/task83_dedup_dryrun_20260922T074620Z.txt / _pairs_20260922T074620Z.json.

AC#1/#3 unaffected by this round, remain checked. AC#2 remains UNCHECKED: still a dry run only -- this round's instructions again forbid mutating production data. Full offline suite (once, at end, this round): 1359 passed, 1 skipped, 0 failed, 1197 deselected (-m "not network"), 389s -- baseline (commit 8bf6d63) was 1336/1/0; delta includes 6 tests I added across both tasks plus other concurrent waves' work in this shared tree. Still In Progress, not Done -- AC#2's merge has not been applied.
