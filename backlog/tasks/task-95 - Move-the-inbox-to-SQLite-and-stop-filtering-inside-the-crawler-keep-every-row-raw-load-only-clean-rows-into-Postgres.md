...
- [x] #6 A full adapter run completes intake end to end with n_new reflecting real new postings, and the 57014 timeout mode is confirmed gone rather than assumed gone
<!-- AC:END -->
...
AC#6 CHECKED 2026-09-23, on today's REAL scheduled run -- not a manually triggered one, per Ivan's steering to prefer live proof over self-measurement. Schedule 1 (daily full pass, 03:00 UTC) fired as run_id=159, trigger=schedule, mode=adapter, scope=clinic (full registry, 407 clinics): status=done, n_rows=14094, n_new=13 real new postings ('resolve: {linked: 0, created: 12, refreshed: 3617}' + link-cross's own 'merged 2 resolve: {created: 0, refreshed: 3615}' -> 12+1=13 matches the run's own n_new). Zero 57014/P0001 anywhere in run 159's log or in today's crawl_issues for board_url='pflege_jobs.cli link-cross' under run 159 specifically -- the crawler-side inbox-INSERT 57014 this AC was written about (A.rest_post('inbox', ...), removed by construction in this task's own earlier rounds) did not recur, as expected since that code path no longer exists.

Schedule 2 (daily verify, 05:17 UTC) also fired today as run_id=160 (queued after 159, started 06:11): status=done, pushed 3017 verdicts (live 2836/gone 65/error 115/blocked 1), 116 unverifiable + 156 city mismatches recorded to crawl_issues for review -- confirms TASK-61's run-109 crash fix holds under today's real conditions too, not just the offline replay.

One honest caveat, scoped OUT of this AC and filed separately: today's crawl_issues also show 4 DIFFERENT 57014 hits (runs 135, 139, 158, 164) inside pflege_jobs.cli link-cross's OWN v_postings read (a different call, OFFSET-paginated, re-running the linked_towns CTE per page) -- this is TASK-124, opened earlier today from the same investigation, not the inbox-insert mode this AC names. Confirmed as genuinely a different failure signature: it hit ad-hoc recrawl runs under concurrent load, not run 159's own scheduled full pass, which completed clean.

All 6 AC now checked. Moving to Done.
<!-- SECTION:NOTES:END -->
...
The crawler queues every row locally in SQLite (data/inbox.sqlite) unfiltered; pflege_jobs.cli cmd_inbox is the single processing step (local queue then Postgres, which stays for the anon-key producers); only finished observations reach Postgres through EdgeSink. Raw rows are marked processed, never deleted, and are reprocessable without re-crawling. Seeded-adapter observations go through the same queue as vendor rows (round 2 fix), resolve_postings() fires once per run not once per page, and clinic_links are pushed once after that resolve (round 3 fix). Proved offline by replaying run_108 (8,743 rows queued, 1,467 to Postgres, 0 to the capped table) and finally by today's real scheduled run: run_id=159 (2026-09-23 03:00 UTC, full registry, trigger=schedule) completed done, 14,094 rows, n_new=13, zero 57014/P0001 on the inbox path this task removed by construction. The daily verify (schedule 2) also completed clean today (3,017 verdicts pushed), confirming the run-109 crash fix (TASK-61) holds live. A separate, different 57014 signature inside link-cross's own v_postings read was found today and filed as TASK-124 -- explicitly out of this task's scope. All 6 AC checked with live and offline evidence; full offline suite last green at 1435 passed, 0 failed.
