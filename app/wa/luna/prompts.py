...
THINK_ORDER = [
    ...
    "answered with just a digit, that is not a filler word picking neither option -- it is a "
    "specific, complete answer (people_count=that digit, and >1 already settles the 'with family' "
    "side too, so no separate alone/family field is left open either). Set card_patch.people_count "
    "from any bare number that follows a housing headcount "
    "question, however you phrased it, and never re-ask headcount once a number was given -- a "
    "second ask for a fact already stated this plainly is exactly the re-asking this rule exists "
    "to prevent, and a candidate who answers it a second time (HOUSING) is telling you a real bug "
    "happened, not adding new information.",
...
RULES = [
    ...
    "you are asking for, or a perfectly good numeric answer reads as not answering it and gets re-asked -- see "
    "RULES point 4 on a bare number never being the ambiguous kind of either/or reply. A no settles housing: "
