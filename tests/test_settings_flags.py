...
def test_stripe_flag_defaults_off(env):
    assert ST.get_feature_flags() == {"stripe": False}
...
def test_save_feature_flags_toggles_and_validates(env):
    assert ST.save_feature_flags({"stripe": True}) == {"stripe": True}
    assert ST.get_feature_flags() == {"stripe": True}
    assert SG.configured() is True
    assert ST.save_feature_flags({"stripe": False, "unknown_key": True}) == {"stripe": False}   # unknown keys ignored
...
def test_get_all_surfaces_flags_and_status_notes(env):
    d = ST.get_all()
    assert d["feature_flags"] == {"stripe": False}
...
def test_put_settings_flags_owner_only(client):
    ...
    assert r.status_code == 200 and r.json() == {"stripe": True}
    assert client.get("/api/settings", headers=h).json()["feature_flags"] == {"stripe": True}
