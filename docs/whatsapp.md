...
**Webhook router (TASK-84):** `app/wa/router.py`, `POST /api/wa/route-webhook` — the dispatch layer TASK-75 itself left out. `route_webhook()` verifies the signature, splits a payload by `route_decision()` per message's phone (one payload can carry messages for several phones/owners at once), processes `'us'` messages through the existing `app.wa.api.handle_payload()` unchanged, and forwards `'them'` messages — reconstructed and re-signed with the shared Meta app secret, so the real system's own signature check still passes — to `WA_REAL_SYSTEM_WEBHOOK_URL`. No URL configured with a `'them'` message present raises loudly rather than dropping a real reply. Mounted in both `app/wa/asgi.py` and `app/main.py`, same two-doors pattern as the main webhook route. **What this deliberately does not do:** register itself as Meta's actual webhook URL. That's a one-time, external configuration change in Meta Business Manager — a separate, explicitly-confirmed step for whoever owns that Meta app, not something this repo's code does on its own.
